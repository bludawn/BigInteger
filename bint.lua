...
--- Check if a number is 2 considering bints.
-- @param x A bint or a lua number.
function bint.istwo(x)
  if getmetatable(x) == bint then
    if x[1] ~= 2 then
      return false
    end
...
--- Check if a number is 3 considering bints.
-- @param x A bint or a lua number.
function bint.isthree(x)
  if getmetatable(x) == bint then
    if x[1] ~= 3 then
      return false
    end
...
function bint.bwrap(x, y)
  ...
  elseif y < BINT_BITS then
    return x:_band(bint.__shl(bint_one(), y):dec())
  end
  return bint_new(x)
end
...
function bint.brol(x, y)
  ...
  if y > 0 then
    return bint.__bor(bint.__shl(x, y), bint.__shr(x, BINT_BITS - y))
  elseif y < 0 then
    return x:bror(-y)
  end
  return x
...
function bint.bror(x, y)
  x, y = bint_assert_convert(x), bint_assert_tointeger(y)
  if y > 0 then
    return bint.__bor(bint.__shr(x, y), bint.__shl(x, BINT_BITS - y))
  elseif y < 0 then
    return x:brol(-y)
  end
  return x
...
local function multiplyKaratsuba(x, y)
  local xlen = validWordLen(x)
  local ylen = validWordLen(y)

  -- print("xlen, ylen", xlen, ylen, x, y)
  -- error('1')
  -- print("xlen, ylen", xlen, ylen)
  if xlen < 12 or ylen < 12 then
    return multiplyNormal(x, y)
  end

  local half = math.floor((math.max(xlen, ylen) + 1) / 2)
  -- print("half", half)
  local xl = getLower(x, half)
  local xh = getUpper(x, half, xlen)
  local yl = getLower(y, half)
  local yh = getUpper(y, half, ylen)
  ...
  local p3 = multiplyKaratsuba( (xh + xl), (yh + yl) ) - p1 - p2

  return fixzero(p1, 2*half) + fixzero(p3, half) + p2
end
...
function bint.__mul(x, y)
  ...
  local ret = multiplyKaratsuba(x, y)
  if xneg and yneg then
    return ret
  elseif xneg then
    return ret:_unm()
...
function bint.udivmod(x, y)
  ...
  local b = numelbit - denolbit
  deno = bint.__shl(deno, b)
  ...
  while b >= 0 do
    ...
    if le then
      ...
      local i = math.floor(b / BINT_WORDBITS) + 1
      quot[i] = bit.bor(quot[i], bit.lshift(1, (b % BINT_WORDBITS)))
    end
    -- shift right the denominator in one bit
    for i=1,denosize-1 do
      deno[i] = bit.bor(bit.rshift(deno[i], 1), bit.band(bit.lshift(deno[i+1], wordbitsm1), BINT_WORDMAX))
    end
...
function bint.__shr(x, y)
  ...
  if y ~= 0 then
    ...
    x[BINT_SIZE] = bit.rshift(x[BINT_SIZE], y)
  end
  return x
end
...
function bint.upowmodx(x, y, m)
  ...
  while not y:iszero() do
    if y:isodd() then
      z = bint_umod(multiplyNormal(z, x), m)
    end
    y:_shrone()
    x = bint_umod(multiplyNormal(x, x), m)
  end
  return z
end

function bint.checknotprime(a, n, x, t)
  local res = bint.upowmodx(a, x, n)
  local last = res
  print("t", t)
  for i=1, t do
    res = res ^ 2 % n
    if res == ONE and last ~= ONE and last ~= n-ONE then
      return true 
    end
    last = res
  end
...
function bint.millerRabin(x, n)
  x = tobint(x)
  if x:iszero() or x:isone() then
    return false
  end

  if x:istwo() or x:isthree() then
    return true
  end

  local s = 0
  local t = x - ONE
  while tobint(t, true):_band(ONE):iszero() do
      s = s + 1
      t:_shrone()
  end
  ...
  local i = 0
  for i=1, n do
    local a = tobint(math.random(1, math.min(n, 1e100)))
    local c = bint.checknotprime(a, x, t, s)
    if c then
      return false
    end
    i = i + 1
